...
def prepare_objects(query_features, query_labels, val_features, val_labels, support_features, support_labels, clip_prototypes, initial_prototypes, initial_predictions, verbose=True):
    ...
    if isinstance(clip_prototypes, list):
        clip_prototypes = [tensor.cuda().float() for tensor in clip_prototypes]
    else:
    ...
    if not isinstance(clip_prototypes, list) and len(clip_prototypes.shape) == 3:  # use more than 1 template
    ...
    if isinstance(clip_prototypes, list):
        # logits_all = []
        # for proto in clip_prototypes:
        #     logits_i = 100 * query_features @ proto
        #     logits_all.append(logits_i)
        # logits_all_softmax = torch.cat(logits_all, -1).softmax(-1)
        # k_sizes = [logit.size(-1) for logit in logits_all]
        # split_logits = torch.split(logits_all_softmax, k_sizes, dim=-1)
        # clip_logits = torch.stack([logit.sum(dim=-1) for logit in split_logits], dim=-1)

        logits_all = []
        for proto in clip_prototypes:
            logits_i = 100 * query_features @ proto
            # _, topk_indices = torch.topk(logits_i.mean(0), 16)
            # logits_i = logits_i[:,topk_indices]
            logits_all.append(logits_i.mean(-1))
        clip_logits = torch.stack(logits_all,-1)
    else:
